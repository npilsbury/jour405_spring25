...
I would draw a line at 18 and a line at 3. The calls are centered around 11 calls per day, and there are a few visible outliers that are most likely a few standard deviations beyond the maen. 
```{r daily-calls-plot}
...
These answers differ from the lines I drew fairly significantly. I put the unusually high number at 18, but the z-scores have determined that above 11 is unusually high. This is surprising since the mean was around 10.4. For unusually low, I put the unusual line at 3, but it would actually be placed at 7. This is less surprsing after looking at the graph again. 
```{r plot-unusual-days}
...
We can also calculate z-scores that account for the day of the week pattern. How would you describe the day with the highest z-score without using the term "z-score"?
 ...
 I would describe it as being very unusual compared to the average number of calls for that day of the week. I think that readers understand what an average is, though I would use the term "average" over "mean" when writing a story. I think I could also say how much larger the number of calls was compared to the average for a particular day. 
...
Potential angles could be reporting on what times of the year typically have more 911 overdoes calls and interviewing people (cops, community members, ER workers) to figure out if there is a reason behind the spikes. I would tell an editor this is important to highlight because it could prompt action for a solution or mitigation strategy. 
Another angle could be analyzing the decline of calls after a spike in January in 2023. It could be interesting to see if the decline matches yearly trends, or if there was a specific reason for the decline-- such as a new program implementation, the arrest of some major dealers, etc. 
For any angle studied, the story could lead to county officials or hosptials pushing for change to combat the number of overdoses. Similarly, I think all stories should consult with EMTs who respond to the calls, cops who investigate drug in the county and community members who witness drug use in their area. 
