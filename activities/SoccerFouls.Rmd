...
```{r}
matches <- matches %>% mutate(goal_dif = team_score - opponent_score)
```
...
```{r}
matches %>% summarize(correlation = cor(fouls, goal_dif, meth="pearson"))
```
...
matches %>% 
    ggplot()+
    geom_point(aes(x=goal_dif, y=fouls))+
    geom_smooth(aes(x=goal_dif, y=fouls), method="lm")
```


### Task 6: Describe the results of both the correlation calculation and the scatterplot.
The correlation coefficient and the scatterplot both show no relationship, or correlation, between the goal differential and the amount of fouls. Since the coefficient was close to 0 and the the data showed no possible line of best fit that displays a relationship, the two topics are unrelated according to our findings. 
