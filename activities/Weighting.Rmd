...
The range is 2.8088 because it goes from 0.2298 to 3.0386.

2. What does it mean when a respondent has a weight of 0.23? What about 3.04?
A weight of 0.23 means that there are a lot of responses from that subgroup, and a weight of 3.04 means there are not a lot of responses from that subgroup. 

3. Why might some respondents be weighted more heavily than others?
Some respondents might be weighted more than others because there are less responses from a subgroup they are a part of. Since it is harder to survery certain demographics, the few respondents pollsters have of the demographic are weighted more. 
...
The rarely/never group showed the biggest difference bewteen weighted and unweighted results. The unweighted results were 3.6 percentage points higher than the weighted results in the "yes" category,2 percentage points lower in the "no" category, and 1.6 percentage points lower in the "unsure" category. 

2. How might these differences affect reporting on likely voter turnout?

The differences may affect reporting on voter turnout because the polling information may vary based on where the reporter is getting the poll from. The decisions the pollsters making about weighting will affect the predicted winner and the voter turnout data. Therefore, reporting will vary as the polls vary. 
...
These differences exist because it is impossible to get a truly accurate estimate of how the population will vote, and sometimes having to weight the data brings us closest to the most accurate result. The differences in polls may depend on how the pollster chooses to weight the data. The differences tell us which groups are under-represented based on which population has the least amount of responses in the data, or an amount that does not reflect the population. The differences in weighting/unweighting show which group is the hardest to get responses from, or which is the smallest in the actual population. 
