...
average_incidents <-total_incidents %>% mutate((`Jan`+`Feb`+`Mar`+`Apr`+`May`+`Jun`+`Jul`+`Aug`+`Sep`+`Oct`+`Nov`+`Dec`)/12)
...
```{r}
june_change <- pg_crime_averages%>%mutate(june change=((`Jun`-pg_crime_averages)/pg_crime_averages)*100,)
```
